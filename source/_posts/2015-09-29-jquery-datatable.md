...
因为公司有一个BDP系统需要一个能展示报表数据的页面，把这个任务交给我，在考虑了jqgrid喝jQuery.DataTable之后，确定选择了jQuery.DataTable，不过两者之前都只用过最简单的方式，所以在这里记录一下我在使用该插件完成工作时候遇到的问题

该插件基于1.0.4，相较于1.0之前的版本，虽然是兼容的，但是有些参数已经修改了。

如下面所示，第二个参数是新版本，第三个参数是旧版本，不过在使用过程中，两者都可以使用
...
jQuery.DataTable.js可以使用非json数据的格式。比如一个List，通过jsp或者velocity模板用foreach循环显示后。通过jQuery.DataTable.js进行包装后能出现分页、搜索等结果。

但是使用非json数据有以下问题

1. 如果记录数过多，可能在进入页面的时候，js还来不及解析，从而造成数据一次性出来后，再被js进行解析，造成一种断裂的感觉

2.  如果使用了非json格式，对于非内置的搜索、条件查询等功能、页面会进行跳转。加上第一点，造成体验不好的

所以可以使用json方式进行解析

## 一：如何使用json数据

jQuery.DataTable支持多种格式的JSON数据
...
```js
{
  ...
  [
        ...
        ["NickName0", 0, "Name0", "pwd0"], 
      
        ["NickName1", 1, "Name1", "pwd1"], 
      
        ["NickName2", 2, "Name2", "pwd2"],
      
       ["NickName3", 3, "Name3", "pwd3"], 
      
        ["NickName4", 4, "Name4", "pwd4"]
]
}

```

因为是数组，所以```无需指定列名```，就可以在前台可以通过下面的这种方式解析出来。
...
但是这种方式，在后台非常难以组装，虽然是json格式。也是JSONObject和JSONArray的组合。但是里面是数组，而不是JSONObject，在后台需要循环List，然后通过JSONArray的add方法进行装入。
...
### 常规json格式：JSONObject.put("data", JSONArray())

这种相比上一种方式，无疑会简单很多。首先，组装比较简单，能比较容易的进行组装前台所需的格式。且需要显示的列也可以自由定制。麻烦就是需要写很多个列名
...
import net.sf.json.JSONObject;
	...
	public JSONArray usersArray(){
		...
		for(int i = 0 ; i<3;i++){
			...
			obj.put("nickname", "NickName"+i);
			obj.put("id", i);
			obj.put("username", "Name"+i);
			obj.put("userpwd", "passwod"+i);
			users.add(obj);
		}
		return users;
	}
	...
	JSONArray array = usersArray();
	obj.put("data", array);
	return new DataTableJsonActionResult(obj);



	//更加简单的方式：List<User>可以直接在后台查询的时候组装成
	JSONArray array = JSONArray.fromObject(List<User>);

```		


此外，该插件还支持嵌套类的方式。  如 { "data": "address.city" }


##如何分页以及非插件参数如何进行传递

默认情况下，该插件是在客户端进行分页，如果需要在服务端进行分页，需要加上如下参数：
...
```js
	...
	$('#example').DataTable({
		//服务端分页
		 "serverSide": true,
		"ajax": {  
		            "url": "/table/ajax",   //url路径
		            "type": "POST",    //请求方式
		            "data": function ( d ) {    //非插件参数
		                d.qdate = $('#datepicker').val();
		            }
		        },
		"columns": [
		            { "data": "id" },
...
如上所示，在发送Ajax请求的时候，会传递很多数据到后台，数据格式如下：

```js

draw:1   //不是页数，而是操作数，非刷新情况下，没点击一次，draw+1
columns[0][data]:id      //数据字段
columns[0][name]:
...
columns[3][data]:userpwd
columns[3][name]:
columns[3][searchable]:true
columns[3][orderable]:true
columns[3][search][value]:    //查询参数
...
https://github.com/DataTables/DataTables/tree/RELEASE_1_9_4
...
http://www.tuicool.com/articles/NBBnum
    ...
    @ResponseBody
    public JSONObject userList(String aoData, DataTablesParam parm,
        ...
        int page = (int) Math.ceil(Integer.parseInt(iDisplayStart)
                / Integer.parseInt(iDisplayLength)) + 1; // 页号
        int pageSize = Integer.parseInt(iDisplayLength); // 每页数据条数
        String sortString = "id.asc,gender.desc";// 如果你想排序的话逗号分隔可以排序多列
        PageBounds pageBounds = new PageBounds(page, pageSize,
                Order.formString(sortString));
        PageList<User> pageList = userService.selectByExample(null, pageBounds);
...
使用方法可以从 5266 、9075、9597行

附：来自http://blog.codepiano.com/pages/jquery-datatables-config-cn.html的文档
